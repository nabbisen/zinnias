...
const RESPONSE_TOKEN_MAX_NUMBER = 700
...
export function solvePrompt(question: string, generateTone: GenerateTone): Part[] {
    const prompt: Part[] = [
        ...
        { text: `【回答条件】解答の最終形と関わりが無いものは、回答に書かない (例えば、あなたの試行錯誤の途中経過など)。 ${responseTokenMaxNumberPrompt(RESPONSE_TOKEN_MAX_NUMBER)} 回答にあなた自身のメッセージは書かない。` },
